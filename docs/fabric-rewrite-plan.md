...
- A minimal `/cropbiomelimiter export-registry` server command exports dimensions, biomes, growable crop block ids, mode names, behavior names, precipitation requirements, summary counts, and a `config_schema` metadata block to `config/cropbiomelimiter/cropbiomelimiter-registry-snapshot.json` for the standalone configuration app. Its existing success response reports the exported dimension, biome, and growable-block counts.
