...
public final class ClientAtlasData {
	...
	public static CropBiomeAtlas create() {
		...
		List<AtlasBiome> biomes = registryAccess.lookup(Registries.BIOME)
	...
	private static List<AtlasBiome> biomes(Registry<Biome> registry) {
		...
		for (ResourceKey<Biome> key : registry.registryKeySet()) {
			registry.get(key).ifPresent(holder -> biomes.add(new AtlasBiome(
					key.identifier(),
					holder,
	...
	private static void addConfiguredCrops(Set<Identifier> crops, DimensionRules rules) {
