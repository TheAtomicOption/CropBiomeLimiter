...
public final class JeiBiomeAtlasCategory implements IRecipeCategory<BiomeAtlasPage> {
	...
	@Override
	public RecipeType<BiomeAtlasPage> getRecipeType() {
	...
	public void setRecipe(IRecipeLayoutBuilder builder, BiomeAtlasPage page, IFocusGroup focuses) {
		...
		builder.addInputSlot(0, 0)
				...
				.addItemStack(AtlasItemStacks.biome(entry.biome(), entry.temperatureDomain()));
	...
	private static void addCropSlots(IRecipeLayoutBuilder builder, List<ResourceLocation> crops, int y) {
		for (int column = 0; column < crops.size(); column++) {
			builder.addOutputSlot(column * 20, y)
					.setStandardSlotBackground()
					.addItemStack(AtlasItemStacks.crop(crops.get(column)));
		}
	}
	...
	@Override
	public ResourceLocation getRegistryName(BiomeAtlasPage page) {
		BiomeAtlasEntry entry = page.entry();
