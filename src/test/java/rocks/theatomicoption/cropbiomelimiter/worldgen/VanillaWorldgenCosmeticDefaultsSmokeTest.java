...
public final class VanillaWorldgenCosmeticDefaultsSmokeTest {
	private static final Set<String> NEWER_THAN_1_21_1_BLOCKS = Set.of(
			"minecraft:bush",
	...
	private static void targetVersionClassifierIncludesVanillaCosmetics() {
		...
		assertTrue(defaultBlocks.contains(BuiltInRegistries.BLOCK.getKey(Blocks.SHORT_GRASS).toString()), "1.21.1 defaults should track short grass");
		assertTrue(defaultBlocks.contains(BuiltInRegistries.BLOCK.getKey(Blocks.TALL_GRASS).toString()), "1.21.1 defaults should track tall grass");
		assertTrue(defaultBlocks.contains(BuiltInRegistries.BLOCK.getKey(Blocks.FERN).toString()), "1.21.1 defaults should track fern");
		assertTrue(defaultBlocks.contains(BuiltInRegistries.BLOCK.getKey(Blocks.LARGE_FERN).toString()), "1.21.1 defaults should track large fern");
		assertTrue(defaultBlocks.contains(BuiltInRegistries.BLOCK.getKey(Blocks.MOSS_BLOCK).toString()), "1.21.1 defaults should track moss block");
		assertTrue(defaultBlocks.contains(BuiltInRegistries.BLOCK.getKey(Blocks.MOSS_CARPET).toString()), "1.21.1 defaults should track moss carpet");
		assertTrue(defaultBlocks.contains(BuiltInRegistries.BLOCK.getKey(Blocks.HANGING_ROOTS).toString()), "1.21.1 defaults should track hanging roots");

		for (String blockId : NEWER_THAN_1_21_1_BLOCKS) {
			assertTrue(!defaultBlocks.contains(blockId), "1.21.1 classifier defaults should not include newer block " + blockId);
		}
	}
	...
	private static void generatedDefaultsExcludeNewerVanillaPlants() throws Exception {
		...
		for (String blockId : NEWER_THAN_1_21_1_BLOCKS) {
			assertTrue(!thresholdJson.contains(blockId), "1.21.1 generated Threshold defaults should not include " + blockId);
			assertTrue(!explicitJson.contains(blockId), "1.21.1 generated Explicit defaults should not include " + blockId);
		}
	}
	...
	private static void generatedExplicitDefaultsKeepVanillaCosmeticsGrowable() throws Exception {
		...
		assertEquals("growable", explicitBehavior(overworld, "minecraft:savanna", "minecraft:short_grass"), "short grass should remain growable in savannas");
