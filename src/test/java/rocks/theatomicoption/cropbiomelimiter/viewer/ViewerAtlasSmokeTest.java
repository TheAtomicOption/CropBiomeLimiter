...
public final class ViewerAtlasSmokeTest {
	...
	private static void customDimensionsUseTheLocalBiomeRegistry() {
		ResourceKey<Level> moon = ResourceKey.create(Registries.DIMENSION, new ResourceLocation("example:moon"));
		...
		assertEquals(moon, atlas.cropEntries().get(0).dimension(), "custom dimension identity should be retained");
		assertEquals(new TemperatureDomain(-2.0F, 4.0F), atlas.cropEntries().get(0).temperatureDomain(), "modded temperatures should define the palette domain");
	}
	...
	private static void mixedModesBuildBothLookupDirections() {
		ResourceLocation wheat = new ResourceLocation("minecraft:wheat");
		ResourceLocation carrots = new ResourceLocation("minecraft:carrots");
	...
	private static void largeGroupsCreateStaticPages() {
		...
		assertEquals(1_000, pages.get(0).total(CropBehavior.GROWABLE), "page headings should keep the full group count");
	}
